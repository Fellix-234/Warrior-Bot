export default {
    ...
    async execute(sock, m, { args, isOwner, prefix }) {
        ...
        if (!botIsAdmin) return await sock.sendMessage(m.key.remoteJid, { text: '❌ I need to be an admin to kick members.' });
        ...
        if (users.length === 0) return await sock.sendMessage(m.key.remoteJid, { text: `❌ Please mention a user or reply to their message. Usage: ${prefix}kick @user` });

        for (let user of users) {
            await sock.groupParticipantsUpdate(m.key.remoteJid, [user], 'remove');
        }

        await sock.sendMessage(m.key.remoteJid, { text: '✅ Member(s) removed successfully.' });
    }
};
